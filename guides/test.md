...
- **`stageMedia` overrides the provider page, and `releaseMedia` restores the first stage's
  ...
  screen. An omitted print or motion axis keeps its effective reading, as do `prefers-color-scheme`
  and `forced-colors`. Any other emulated feature the provider configured is cleared. An empty `{}`
