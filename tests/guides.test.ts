...
type InvokeFunction = typeof invokeUnchecked

class ScriptedLoader implements EventSourceInterface<LoaderEvents> {
	readonly #listeners: RecorderInterface<readonly [event: keyof LoaderEvents, handler: unknown]>
	readonly #invoke: InvokeFunction

	constructor(
		listeners: RecorderInterface<readonly [event: keyof LoaderEvents, handler: unknown]>,
	...
	emit(event: keyof LoaderEvents, args: readonly unknown[]): void {
		for (const [name, handler] of this.#listeners.calls) {
			if (name === event) this.#invoke<void>(this, handler, args)
		}
	}
}
...
function parseSchema(
	value: unknown,
	guard: (candidate: unknown) => candidate is Readonly<Record<string, unknown>>,
): Schema | undefined {
	if (!guard(value)) return undefined
...
await new GuideCommand({
	...
	const { createAsyncSource, createStreamSource, isSerializableRecord, ROUTED_FENCES } =
		await import('./setup.js')
	const { afterEach, describe, expect, it } = await import('vitest')

	it('manifest lists the package guide', () => {
	...
	// paired title `findDrift` compares nothing and the case passes on the summaries alone.
	// This pins the population this repository's guide contributes, so removing
	// every `@example` title reddens the suite instead of quietly retiring the gate.
	it('pairs a titled example across the guide and the source', () => {
		expect(report.examples.titles.filter((finding) => finding.spec === GUIDE_SPEC)).toEqual([])
	})

	// The README pitch must equal the guide tagline. `README.md` is outside the concept index,
	...
	for (const { entry, guide, source } of rows) {
		describe(`${entry.concept}`, () => {
			...
			// The equality gate compares each `Summary` cell and titled fence with its source.
			// Converge through the native entry; never weaken this assertion. Select source
	...
	describe('guide fences', () => {
		...
		it('carries every fence-bearing guide heading in exactly one place', () => {
			...
			const carrier = requireValue(
				files['tests/guides.test.ts'],
				'Missing carrier: tests/guides.test.ts',
			)
			const transcribed = discovered.filter((name) =>
				carriesMarker(carrier, requireValue(markers.get(name))),
			)
		...
		it('keys one recorder per named event and records each delivery in order', () => {
			const loader = new ScriptedLoader(
				createRecorder<readonly [event: keyof LoaderEvents, handler: unknown]>(),
				invokeUnchecked,
			)
		...
		it('reproduces the canonical wire after a parse across an untrusted JSON boundary', () => {
			...
			const received = requireValue(parseSchema(JSON.parse(wire), isSerializableRecord))
		...
		it('keys a walk workspace-relative, takes a named file whatever the filter says, and excludes below a directory', () => {
			const workspace = resolveRoot(import.meta)

			expect(
				Object.keys(readInventory(workspace, ['src/core'], { extensions: ['.ts'] })),
			).toStrictEqual([
			...
			expect(
				Object.keys(
					readInventory(workspace, ['package.json', 'src/core'], { extensions: ['.ts'] }),
				),
			).toStrictEqual([
				'package.json',
				'src/core/constants.ts',
			...
			expect(
				Object.keys(
					readInventory(workspace, ['src/core'], {
						extensions: ['.ts'],
			...
			expect(
				Object.keys(
					readInventory(workspace, ['src'], { extensions: ['.ts'], exclude: ['src/server'] }),
				),
			).toStrictEqual([
			...
			expect(
				readInventory(workspace, ['src/core/index.ts'], {
					extensions: ['.ts'],
					exclude: ['src/core'],
				}),
			).toStrictEqual({})
		...
		it('reads back what it wrote, refuses an escape, and nests one allocation in another', () => {
			...
			try {
				...
				expect(scratch.write('src/notes.ts', 'export {}\n')).toBe(
					join(scratch.path, 'src/notes.ts'),
				)
		...
		it('resolves a contained path and refuses a relative and an absolute escape', () => {
			...
			try {
				expect(resolveContained(base, 'src/index.ts')).toBe(join(base, 'src/index.ts'))
				expect(resolveContained(base, join(base, 'src/index.ts'))).toBe(join(base, 'src/index.ts'))
				expect(resolveContained(base, '../escape.ts')).toBeUndefined()
				expect(resolveContained(base, join(base, '../escape.ts'))).toBeUndefined()
				expect(resolveContained(base, '/etc/passwd')).toBeUndefined()
			} finally {
				scratch.destroy()
			}
		})
	...
	describe('README examples', () => {
		...
		it('states what each readInventory call it shows really returns', () => {
			const walked = readInventory(workspace, ['src/core', 'src/server'], { extensions: ['.ts'] })
			const text = normalizeComments(readme)

			expect(text).toContain(normalizeComments(renderKeys(Object.keys(walked))))

			expect(text).toContain(
				normalizeComments(JSON.stringify(requireValue(walked['src/core/index.ts']))),
			)
